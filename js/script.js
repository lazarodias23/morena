...
function createModelCard(model) {
    ...
    // Changed class: bg-zinc-900 -> glass-card
    return `
    <div class="group glass-card rounded-xl overflow-hidden hover:border-brand-red transition-all duration-300 cursor-pointer fade-in" onclick="openModelModal('${model.id}')">
        <div class="relative aspect-[3/4] overflow-hidden">
            <img src="${model.coverImage}" alt="${model.name}" loading="lazy" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
            ...
            <div class="absolute bottom-0 left-0 p-6 w-full">
                <span class="text-brand-red text-xs font-bold uppercase tracking-widest mb-1 block drop-shadow-md">${model.category}</span>
                <h3 class="text-2xl font-serif text-white group-hover:text-brand-red transition-colors drop-shadow-lg">${model.name}</h3>
...
if (document.getElementById('models-grid')) {
    ...
    function renderFilteredModels() {
        ...
        if (filtered.length > 0) {
            ...
            grid.innerHTML = `
                <div class="col-span-full text-center py-20 glass-panel rounded-xl">
                    <p class="text-xl text-zinc-400 font-serif mb-4">Nenhuma acompanhante encontrada com estes filtros.</p>
    ...
    function updateUI() {
        ...
        document.querySelectorAll('.filter-region-btn').forEach(btn => {
            if (btn.dataset.region === activeRegion) {
                ...
                btn.className = 'filter-region-btn px-4 py-2 rounded-full text-xs uppercase font-bold tracking-wide transition-all bg-black/40 text-zinc-400 hover:bg-white/10 hover:text-white border border-white/5';
            }
        });
        ...
        document.querySelectorAll('.filter-service-btn').forEach(btn => {
            ...
            if (isActive) {
                ...
                btn.className = `${baseClass} border-white/10 bg-transparent text-zinc-400 hover:border-white/30`;
            }
        });
...
if (document.getElementById('agency-carousel')) {
    ...
    modelsData.forEach(model => {
        ...
        card.className = "min-w-[280px] w-[280px] sm:min-w-[320px] snap-center glass-card rounded-xl overflow-hidden hover:border-brand-red transition-all duration-300 cursor-pointer group";
        ...
        card.innerHTML = `
            <div class="h-96 relative overflow-hidden">
                ...
                <div class="absolute bottom-0 left-0 p-6">
                    <h3 class="text-2xl font-serif text-white drop-shadow-lg">${model.name}</h3>
                    <p class="text-brand-red text-xs font-bold uppercase tracking-widest drop-shadow-md">${model.category}</p>
                </div>
            </div>
        `;
